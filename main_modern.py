#!/usr/bin/env python3
"""
Telegram SMS Bot - Modern Version with Latest Libraries
Uses proper async initialization patterns for python-telegram-bot 21.0+
"""
...
async def notify_admins_api_failure(context: ContextTypes.DEFAULT_TYPE, failure_type):
...
async def check_sms_for_number(phone_number, date_str, context=None):
                ...
                if response.status == 200:
                    ...
                    if context:
                        if 'direct script access not allowed' in response_text.lower():
                            asyncio.create_task(notify_admins_api_failure(context, "access_blocked"))
                        else:
                            asyncio.create_task(notify_admins_api_failure(context, f"HTTP {response.status}"))
                    return None
    except asyncio.TimeoutError:
        if context:
            asyncio.create_task(notify_admins_api_failure(context, "connection_timeout"))
        return None
    except Exception as e:
        if context:
            asyncio.create_task(notify_admins_api_failure(context, f"connection_error: {str(e)}"))
        return None
...
async def send_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result and "number" in result:
            ...
            asyncio.create_task(start_otp_monitoring(context, user_id, number, query.message.message_id))
...
async def start_otp_monitoring(context: ContextTypes.DEFAULT_TYPE, user_id, number, message_id):
    """Start monitoring for OTPs on a number"""
    try:
        formatted_number = format_number_display(number)
        ...
        logger.info(f"Starting OTP monitoring for {formatted_number} (User: {user_id})")
        
        while datetime.now() < end_time:
            try:
                date_str = datetime.now(TIMEZONE).strftime("%Y-%m-%d")
                sms_data = await check_sms_for_number(number, date_str, context)
                
                if sms_data and sms_data.get('aaData'):
                    for row in sms_data['aaData'][:5]:  # Check last 5 messages
                        if len(row) >= 3:
                            ...
                            if otp:
                                # Found OTP! Send to user and cleanup
                                otp_message = f"📞 Number: {formatted_number}\n🔐 {sender}: {otp}"
                                ...
                                # Clean the number from database
                                db = context.bot_data["db"]
                                coll = db[COLLECTION_NAME]
                                await coll.delete_one({"number": number})
                                
                                logger.info(f"OTP found and sent for {formatted_number}: {otp}")
                                return
                
                await asyncio.sleep(check_interval)
                
            except Exception as e:
                logger.error(f"Error during OTP monitoring for {number}: {e}")
                await asyncio.sleep(check_interval)
        
        # Timeout reached
        timeout_message = f"⏰ Morning call ended for {formatted_number}"
        await context.bot.send_message(chat_id=user_id, text=timeout_message)
        logger.info(f"OTP monitoring timeout for {formatted_number}")
        
    except Exception as e:
        logger.error(f"Error in OTP monitoring task for {number}: {e}")
...
• Admin notifications for API issues
...
async def check_api_connection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        start_time = datetime.now()
        result = await check_sms_for_number(test_number, date_str, context)
...
async def clear_cache(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Countries cache cleared successfully!")

async def background_otp_cleanup_task(context: ContextTypes.DEFAULT_TYPE):
    """Background task to cleanup OTPs"""
    ...
    while True:
        try:
            ...
            current_time = datetime.now(TIMEZONE)
            logger.debug(f"Running background cleanup at {current_time}")
            
            # Here you would implement the cleanup logic
            # This is a simplified version - full implementation would check for OTPs
...
async def post_init(application: Application) -> None:
    """Initialize background tasks after application startup"""
    logger.info("🔄 Starting background cleanup task...")
    try:
        # Start background cleanup task
        cleanup_task = asyncio.create_task(background_otp_cleanup_task(application))
...
async def main():
    ...
    try:
        logger.info("🚀 Starting Modern Telegram Bot...")
        
        # Build application with proper post_init
        ...
        application.add_handler(CommandHandler("checkapi", check_api_connection))
        ...
        application.add_handler(CallbackQueryHandler(request_number, pattern="request_number"))
        application.add_handler(CallbackQueryHandler(send_number, pattern="^country_"))
        application.add_handler(CallbackQueryHandler(show_sms, pattern="^sms_"))
        application.add_handler(CallbackQueryHandler(menu, pattern="^menu$"))
        
        logger.info("✅ Bot initialized successfully")
        logger.info("📱 Bot started and polling...")
        
        # Start the bot with async context
